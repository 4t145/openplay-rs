...
use openplay_basic::{
    ...
    room::{
        Chat, Room, RoomEvent, RoomObserverState, RoomObserverView, RoomPlayerState, RoomUpdate, RoomUserPosition,
        Update, UserActionEvent,
    },
    user::{
        Action, ActionSource, DynUserAgent, User, UserId,
        game_action::GameActionData,
        room_action::{JoinRoom, PositionChange, ReadyStateChange, RoomActionData, RoomManage},
    },
};
        ...
        match action {
            RoomActionData::Join(join) => {
                ...
                if !in_players && !in_observers {
                    ...
                    room.state.observers.insert(user_id.clone(), RoomObserverState {
                        is_connected: true,
                    ...
                    // User already in room (reconnect scenario): push current state
            ...
            RoomActionData::PositionChange(change) => {
                ...
                let valid = match &change.from {
                    RoomUserPosition::Observer(_) => {
                        room.state.observers.contains_key(user_id)
                    }
                    RoomUserPosition::Player(pos) => {
                        room.state.players.get(pos).map_or(false, |p| p.player.id == *user_id)
                    }
                };

                if !valid {
                    tracing::warn!("PositionChange: user {:?} not at claimed 'from' position {:?}", user_id, change.from);
                    return Ok(None);
                }
                ...
                match (&change.from, &change.to) {
                    ...
                    (RoomUserPosition::Observer(_), RoomUserPosition::Player(target_pos)) => {
                        ...
                        if room.state.players.contains_key(target_pos) {
                            tracing::warn!("PositionChange: target seat {:?} is occupied", target_pos);
                            return Ok(None);
                        }
                        // Remove from observers
                        if let Some(obs) = room.state.observers.remove(user_id) {
                            // Insert as player
                            room.state.players.insert(target_pos.clone(), RoomPlayerState {
                                id_ready: false,
            ...
            RoomActionData::RoomManage(manage) => {
                ...
                if room.info.owner == *user_id {
                    match manage {
                        ...
                        RoomManage::StartGame => {
                            ...
                            if room.state.player_count() < 3 {
                                tracing::warn!("StartGame: not enough players ({}/3)", room.state.player_count());
                                return Ok(None);
                            }
                    ...
                    tracing::warn!(
                        ...
                        user_id, room.info.owner, manage
                    );
                }
            }
