...
use crate::global_config::theme_manager::{PokerThemeId, PokerThemeRegistry, PokerThemeStore};
...
pub struct PokerTextureUpdateSet;

impl Plugin for PokerPlugin {
    fn build(&self, app: &mut App) {
        app.configure_sets(Update, PokerTextureUpdateSet)
            .add_systems(
                Update,
                update_poker_card_texture.in_set(PokerTextureUpdateSet),
            )
            .add_systems(Update, (interact_poker_card, animate_poker_card_tilt));
    }
}
            ...
            for (card_transform, mut tilt) in card_query.iter_mut() {
                ...
                if denom.abs() > 1e-6 {
                    ...
                    if t >= 0.0 {
                        ...
                        // 将交点转换到卡片局部空间
                        let local_point = card_transform
                            .affine()
                            .inverse()
                        ...
                        if local_point.x.abs() < half_width && local_point.y.abs() < half_height {
                            ...
                            tilt.target_rotation =
                                Quat::from_euler(EulerRot::XYZ, rot_x, rot_y, 0.0);
                        } else {
...
pub fn animate_poker_card_tilt(
    ...
    for (mut transform, tilt) in query.iter_mut() {
        ...
        transform.rotation = transform
            .rotation
            .slerp(tilt.target_rotation, time.delta_secs() * 15.0);
    }
}
